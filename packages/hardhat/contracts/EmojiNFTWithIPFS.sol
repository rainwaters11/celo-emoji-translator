    ...
    ERC721Burnable
{
    ...
    // Track whether to use IPFS or on-chain metadata
    bool public useIPFSMetadata = true;
    ...
    mapping(uint256 => string) private _ipfsURIs; // Store IPFS URIs
    ...
    ) ERC721("Celo Emoji NFT with IPFS", "CEMOJI") Ownable(initialOwner) {}
    ...
    function setUseIPFSMetadata(bool _useIPFS) public onlyOwner {
        useIPFSMetadata = _useIPFS;
    }

    // Main minting function with IPFS URI
    function mintEmojiNFTWithIPFS(
        address to,
        ...
        _ipfsURIs[tokenId] = ipfsURI;

        _safeMint(to, tokenId);
        
        // Set the token URI (IPFS or on-chain)
        string memory uri = useIPFSMetadata ? ipfsURI : generateTokenURI(tokenId, originalText, emojiMessage);
        _setTokenURI(tokenId, uri);

        emit EmojiNFTMinted(to, tokenId, originalText, emojiMessage, uri);
    }

    // Legacy minting function (on-chain only)
    function mintEmojiNFT(
        ...
        // Generate and set the token URI (always on-chain for this function)
